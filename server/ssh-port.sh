...
# Additional validation if passed as argument
if ! [[ "$new_port" =~ ^[0-9]+$ ]] || [ "$new_port" -lt 1 ] || [ "$new_port" -gt 65535 ]; then
    echo -e "\e[31mInvalid port number provided as argument. Must be between 1 and 65535.\e[0m"
    exit 1
fi
...
if is_port_in_use "$new_port"; then
    echo -e "\e[31mPort $new_port is already in use.\e[0m"
    exit 1
fi
