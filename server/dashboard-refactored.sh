...
cleanup_on_exit() {
    ...
    if [[ $exit_code -ne 0 ]]; then
        ...
        echo "❌ Скрипт завершился с ошибкой (код: $exit_code)" >&2
        echo "📋 Подробности в лог-файле: $LOG_FILE" >&2
...
perform_installation() {
    ...
    log_info "Выполняем резервное копирование..."
    backup_existing_motd
    log_info "Резервное копирование завершено успешно"
    
    # Удаление существующих MOTD файлов
    log_info "Начинаем удаление существующих MOTD файлов..."
    remove_existing_motd
    log_info "Удаление MOTD файлов завершено"
    ...
    log_info "Создаем dashboard файл..."
    local tmp_dashboard
    tmp_dashboard=$(create_dashboard_file)
    log_info "Dashboard файл создан: $tmp_dashboard"
    
    # Предпросмотр
    log_info "Показываем предпросмотр..."
    echo "===================================================="
    echo "📋 Предпросмотр NeoNode MOTD (оптимизированная версия):"
    echo "===================================================="
    if bash "$tmp_dashboard" 2>/dev/null; then
        log_info "Предпросмотр выполнен успешно"
    else
        log_warning "Предпросмотр завершился с ошибками, но продолжаем установку"
    fi
    echo "===================================================="
